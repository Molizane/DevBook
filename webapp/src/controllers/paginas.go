...
// CarregarTelaDeLogin monta a tela de login
...
// CarregarPaginaDeCadastroDeUsuario monta a tela cadastro de usuários
...
// CarregarPaginaDeEdicaoDoUsuarioLogado carrega a página de edição dos dados do usuário
func CarregarPaginaDeEdicaoDoUsuarioLogado(w http.ResponseWriter, r *http.Request) {
	cookie, _ := cookies.Ler(r)
	usuarioID, _ := strconv.ParseUint(cookie["id"], 10, 64)
	...
	if usuario.ID == 0 {
		respostas.JSON(w, http.StatusInternalServerError, respostas.ErroAPI{Erro: "Erro ao bsucar usuário"})
		return
	}
...
// CarregarPaginaDeAtualizacaoDeSenha carrega a página para atualização de senha do usuário
